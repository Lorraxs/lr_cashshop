...
function ESX.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if GetResourceState('buzzer-progressBar') ~= 'started' then error('progressbar needs to be started in order for QBCore.Functions.Progressbar to work') end
    exports['buzzer-progressBar']:Progress({
        name = name:lower(),
        duration = duration,
        label = label,
...
function ESX.ShowNotification(text, textype, length)
    if type(textype) == "number" then
        textype = 'primary'
    end
    
    if type(textype) == "boolean" then
        textype = 'primary'
    end

    exports['esx_notifyv2']:Notify(text, textype, length)
end
...
function ESX.ShowAdvancedNotification(sender, subject, msg, textureDict, iconType, flash, saveToBrief, hudColorIndex)
    ...
    AddTextEntry('esxAdvancedNotification', "<FONT FACE='Balo'>"..msg)
...
function ESX.ShowHelpNotification(msg, thisFrame, beep, duration)
    AddTextEntry('esxHelpNotification', "<FONT FACE='Balo'>"..msg)
...
function ESX.ShowFloatingHelpNotification(msg, coords)
    AddTextEntry('esxFloatingHelpNotification', "<FONT FACE='Balo'>"..msg)
...
if GetResourceState("buzzer-evilsGovMenu") ~= "missing" then
    function ESX.OpenContext2(...)
        exports["buzzer-evilsGovMenu"]:Open(...)
    end

    function ESX.PreviewContext2(...)
        exports["buzzer-evilsGovMenu"]:Preview(...)
    end

    function ESX.CloseContext2(...)
        exports["buzzer-evilsGovMenu"]:Close(...)
    end

    function ESX.RefreshContext2(...)
       exports["buzzer-evilsGovMenu"]:Refresh(...) 
    end
else 
    function ESX.OpenContext2()
        print("[^1ERROR^7] Tried to ^5open^7 context menu, but ^5buzzer-evilsGovMenu^7 is missing!")
    end

    function ESX.PreviewContext2()
        print("[^1ERROR^7] Tried to ^5preview^7 context menu, but ^5buzzer-evilsGovMenu^7 is missing!")
    end

    function ESX.CloseContext2()
        print("[^1ERROR^7] Tried to ^5close^7 context menu, but ^5buzzer-evilsGovMenu^7 is missing!")
    end

    function ESX.RefreshContext2()
        print("[^1ERROR^7] Tried to ^5Refresh^7 context menu, but ^5buzzer-evilsGovMenu^7 is missing!")
    end
end
...
function ESX.SetPlate(vehicle, plate)
    SetVehicleNumberPlateText(vehicle, Trim(plate))
end
...
function ESX.Game.SetVehicleProperties(vehicle, props)
    ...
    if props.plate ~= nil then
        ESX.SetPlate(vehicle, props.plate)
    end
        ...
        for _, itemData in pairs(ESX.PlayerData.inventory) do
            if isTable then
                for k, v in pairs(items) do
                    local itemKV = {k, v}
                    if itemData and itemData.name == itemKV[kvIndex] and ((amount and itemData.count >= amount) or (not isArray and itemData.count >= v) or (not amount and isArray)) then
                        count += 1
                    end
                end
                ...
                if itemData and itemData.name == items and (not amount or (itemData and amount and itemData.count >= amount)) then
                    return true
                end
            end
        end
...
function ESX.Logs(name, message)
    TriggerServerEvent('esx_log:server:CreateLog', name, message)
end
...
RegisterNetEvent('esx:notify', function(msg, type, length)
    ESX.ShowNotification(msg, type, length)
end)
...
FontId = RegisterFontId('Balo') -- the name from the .xml

AddTextEntry('CUSTOM_STRING', "<FONT FACE='Balo'>~a~</FONT>")
AddTextEntry('STRING', "<FONT FACE='Balo'>~a~</FONT>")