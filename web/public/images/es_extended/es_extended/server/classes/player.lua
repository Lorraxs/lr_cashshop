...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, loadout, name, coords, hwid)
	...
	stateBag:set("name", GetPlayerName(self.source), true)
	...
	function self.setMoney(money, reason)
	...
	function self.hasPermission(rank)
		return (self.getGroup() == rank)
	end
	...
	function self.setName(newName)
		self.name = newName
		Player(self.source).state:set("name", GetPlayerName(self.source), true)
	end
	...
	function self.setAccountMoney(accountName, money, reason)
		...
		if money >= 0 then
			...
			if account then
				...
				ESX.Logs('money', '['..self.source..']['..self.identifier..'] '..GetPlayerName(self.source)..' | set '..money..'$ ('..accountName..': '..self.accounts[account.index].money..') | from : '..(reason or 'Không rõ')..'')
	...
	function self.addAccountMoney(accountName, money, reason)
		...
		if money > 0 then
			...
			if account then
				...
				ESX.Logs('money', '['..self.source..']['..self.identifier..'] '..GetPlayerName(self.source)..' | +'..money..'$ ('..accountName..': '..self.accounts[account.index].money..') | from : '..(reason or 'Không rõ')..'')
			else 
	...
	function self.addInventoryItem(name, count, reason)
		local item = self.getInventoryItem(name)

		if item then
			if self.canCarryItem(name, count) then
				...
				ESX.Logs('additem', '['..self.source..']['..self.identifier..'] '..GetPlayerName(self.source)..' | +'..count..' '..name..' (x'..item.count..') | from : '..(reason or 'Không rõ')..'')
			else
				TriggerClientEvent('esx:showNotification', self.source, 'Kho đồ đã đầy', 'error')
			end
		end
	end

	function self.removeInventoryItem(name, count, reason)
		local item = self.getInventoryItem(name)
		...
		if item then
			...
			if count > 0 then
				...
				if newCount >= 0 then
					...
					ESX.Logs('removeitem', '['..self.source..']['..self.identifier..'] '..GetPlayerName(self.source)..' | -'..count..' '..name..' (x'..item.count..') | from : '..(reason or 'Không rõ')..'')
				end
			else
	...
	function self.setJob2(job, grade)
		...
		if ESX.DoesJob2Exist(job, grade) then
			local jobObject, gradeObject = ESX.Job2s[job], ESX.Job2s[job].grades[grade]
