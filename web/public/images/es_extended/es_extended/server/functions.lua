...
function Core.SavePlayer(xPlayer, cb)
  ...
  MySQL.prepare(
    'UPDATE `users` SET `accounts` = ?, `job` = ?, `job2` = ?, `job_grade` = ?, `job2_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ? WHERE `identifier` = ?',
    parameters,
...
RegisterCommand('getpacket', function(source)
  if source == 0 then

    print('packetlength: ', #ESX.Players, ESX.DumpTable(ESX.Players))
    print(test, timemer)
  end
end)

RegisterCommand('getpacket2', function(source, args)
  local xPlayer = ESX.GetPlayerFromId(tonumber(args[1]))

  print(ESX.DumpTable(xPlayer.inventory))
end)
...
function ESX.DoesJob2Exist(job, grade)
  ...
  if job and grade then
    if ESX.Job2s[job] and ESX.Job2s[job].grades[grade] then
      return true
    end
  end
...
function ESX.Logs(name, message)
  TriggerEvent('esx_log:server:CreateLog', name, message)
end