...
function loadESXPlayer(identifier, playerId, isNew)
  ...
  local job, job2, grade, grade2, jobObject, job2Object, gradeObject, grade2Object = result.job, result.job2, tostring(result.job_grade), tostring(result.job2_grade)
  ...
  if ESX.DoesJob2Exist(job2, grade2) then
    job2Object, grade2Object = ESX.Job2s[job2], ESX.Job2s[job2].grades[grade2]
  else
  ...
  local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.job2,
    userData.loadout, userData.playerName, userData.coords, userData.hwid)
    ...
    if type == 'item_standard' then
      ...
      if itemCount > 0 and sourceItem.count >= itemCount then
        if targetXPlayer.canCarryItem(itemName, itemCount) then
          ...
          targetXPlayer.addInventoryItem(itemName, itemCount, 'esx:giveInventoryItem')
    ...
    if pickup then
      if pickup.type == 'item_standard' then
        if xPlayer.canCarryItem(pickup.name, pickup.count) then
          xPlayer.addInventoryItem(pickup.name, pickup.count, 'esx:onPickup')
          success = true
        else
