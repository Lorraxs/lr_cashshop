...
ESX.RegisterCommand('car', 'admin', function(xPlayer, args, showError)
	...
	ESX.OneSync.SpawnVehicle(args.car, playerCoords, playerHeading, upgrades, function(networkId)
		if networkId then
			...
			while #ESX.GetPlate(vehicle) == 0 do
				Wait(10)
			end

			TriggerClientEvent('vehiclekeys:client:SetOwner', xPlayer.source, ESX.GetPlate(vehicle))
		end
	end)
...
ESX.RegisterCommand({'giveaccountmoney', 'givemoney'}, 'admin', function(xPlayer, args, showError)
	if args.playerId.getAccount(args.account) then
		args.playerId.addAccountMoney(args.account, args.amount, "admin đưa tiền")
	else
		showError(TranslateCap('command_giveaccountmoney_invalid'))
	end
...
if not Config.OxInventory then
	ESX.RegisterCommand('giveitem', 'admin', function(xPlayer, args, showError)
		args.playerId.addInventoryItem(args.item, args.count, 'admin give')
